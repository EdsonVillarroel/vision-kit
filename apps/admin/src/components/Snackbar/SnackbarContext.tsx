...
export const SnackbarProvider: React.FC<SnackbarProviderProps> = ({ children }) => {
  ...
  const showError = useCallback((message: string) => showSnackbar(message, 'error', 5000), [showSnackbar]);
...
const SnackbarItem: React.FC<SnackbarItemProps> = ({ message, type, onDismiss }) => {
  const config = {
    success: { bgGradient: 'bg-gradient-to-r from-green-600 to-green-700', iconBg: 'bg-green-500',
    ...
    error: { bgGradient: 'bg-gradient-to-r from-red-600 to-red-700', iconBg: 'bg-red-500',
    ...
    warning: { bgGradient: 'bg-gradient-to-r from-yellow-500 to-yellow-600', iconBg: 'bg-yellow-400',
    ...
    info: { bgGradient: 'bg-gradient-to-r from-theme-primary to-theme-dark-primary', iconBg: 'bg-theme-accent',
  ...
  const { bgGradient, iconBg, icon } = config[type];

  return (
    <div className={`${bgGradient} text-white rounded-2xl shadow-2xl min-w-[320px] max-w-md animate-slideIn backdrop-blur-sm border border-white/20 overflow-hidden`}>
      <div className="flex items-center gap-4 px-5 py-4">
        <div className={`${iconBg} rounded-xl p-2 shadow-lg`}>{icon}</div>
        <p className="font-semibold text-sm flex-1">{message}</p>
        <button onClick={onDismiss} className="ml-2 hover:bg-white/20 rounded-xl p-2 transition-all duration-300" aria-label="Dismiss">
