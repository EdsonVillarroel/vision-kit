...
export const MainLayout: React.FC<MainLayoutProps> = ({ children, menuItems, navbar }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className={clsx('transition-all duration-300 ease-in-out', isCollapsed ? 'lg:ml-20' : 'lg:ml-64')}>
        {navbar && (
          <nav className="bg-gradient-to-r from-theme-dark-primary via-theme-primary to-theme-dark-primary shadow-2xl border-b border-theme-accent/20 fixed top-0 left-0 right-0 z-40 backdrop-blur-md">
            <div className={clsx('px-4 sm:px-6 lg:px-8 transition-all duration-300', isCollapsed ? 'lg:ml-20' : 'lg:ml-64')}>
              <div className="flex justify-between items-center h-16">
                <div className="flex items-center gap-4">
                  <button
                    onClick={() => {}}
                    className="lg:hidden p-2.5 rounded-xl text-theme-text-icons hover:bg-white/15 transition-all duration-300"
