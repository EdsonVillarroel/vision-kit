      ...
      {item.path && !hasChildren ? (
        <Link
          ...
          className={clsx(
            ...
            level > 0 && 'pl-12 ml-2',
            isActive
              ? 'bg-gradient-to-r from-theme-primary to-theme-dark-primary text-theme-text-icons font-bold shadow-lg shadow-theme-primary/30 scale-[1.02]'
        ...
        >
          {content}
          {isActive && <div className="absolute right-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-theme-accent rounded-l-full" />}
        </Link>
      ) : (
        <button
          onClick={handleClick}
          className={clsx(
            ...
            level > 0 && 'pl-12 ml-2',
            isExpanded
              ? 'text-theme-dark-primary font-bold bg-gradient-to-r from-theme-light-primary/60 to-theme-light-primary/40 shadow-md'
  ...
  return (
    <>
      ...
      <aside className={clsx(
        ...
        {/* Header */}
        <div className="h-16 flex items-center justify-between px-4 border-b border-theme-divider/50 bg-gradient-to-r from-theme-dark-primary to-theme-primary shadow-lg">
          {!isCollapsed && (
            <div className="flex items-center gap-2.5">
              <div className="w-9 h-9 bg-gradient-to-br from-theme-accent to-theme-primary rounded-xl flex items-center justify-center text-white font-bold shadow-lg">
                VK
              </div>
              <div>
                <h2 className="text-sm font-bold text-theme-text-icons leading-tight">Vision Kit</h2>
                <p className="text-xs text-theme-text-icons/60 leading-tight">Platform Admin</p>
              </div>
            </div>
          )}
          {isCollapsed && (
            <div className="w-9 h-9 bg-gradient-to-br from-theme-accent to-theme-primary rounded-xl flex items-center justify-center text-white font-bold shadow-lg mx-auto">
              VK
            </div>
          )}
          <button
            onClick={toggleCollapse}
            className="p-2.5 hover:bg-white/10 rounded-xl transition-all duration-300 hidden lg:block text-theme-text-icons hover:shadow-md"
            aria-label="Toggle sidebar"
          >
            <svg className={clsx('w-5 h-5 transition-transform duration-300', isCollapsed && 'rotate-180')} fill="none" stroke="currentColor" viewBox="0 0 24 24">
